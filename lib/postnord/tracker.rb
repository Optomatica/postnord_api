require 'json'
require 'postnord/parcelTracker.rb'
require 'postnord/pdf.rb'
class Tracker<Base
  attr_accessor :href
  attr_accessor :id
  ...
  def self.CreateInstances(json_string)
    ...
    trackers = trackersJson.map do |trackerJson|
      ...
      if trackerJson['parcels']
        tracker.parcels = trackerJson['parcels'].map { |parcelsJson| ParcelTracker.new(parcelsJson) }
      end

      if trackerJson['pdfs']
        tracker.pdfs = trackerJson['pdfs'].map { |pdfJson| PostnordPDF.new(pdfJson) }
      end
