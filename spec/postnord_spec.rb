...
describe Postnord do
  ...
  it "shipment is working" do
    ...
    s.setReciver(name:"Postnord AB",phone:"+46 8 34 35 15",email:"sales@unifaun.com",address1:"Tegnérgatan 34",address2:"test address2",zipcode:"11359",city:"cairo",country:"SE")
    s.setService("PUA")
    ...
    # s.save(pdfConfig)
    tracker = Postnord.createShipment(s,pdfConfig)
    p tracker
    f=File.new("file.pdf", "w")
    f.write(Postnord.getShipmentPDF(tracker.pdfs[0].href))
    f.close
    # p s.to_json
  end
  ...
  it "shipment with customsDeclaration " do
    ...
    for i in 0..(tracker.pdfs.count-1)
      ...
      f.write(Postnord.getShipmentPDF(tracker.pdfs[i].href))
      f.close
    end
    # p s.to_json
  end
